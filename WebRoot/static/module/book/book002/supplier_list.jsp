<%@page import="com.ambow.model.Supplier"%>
...
<body onload="init()">
	...
	<ul id="button">
		<li><a href="<%=path %>/static/module/book/book002/supplier_add.jsp">新 建</a></li>
	</ul>

	<table width="90%" border="0" align="center" cellpadding="0"
		cellspacing="0" class="table2">
		<form action="<%=path%>/SupplierListServlet" method="post"
			name="form2" id="form2">
			<tr>
				<td width="120" align="center" nowrap="nowrap">输入供应商名称 ：</td>
	...
	<table width="90%" border="0" align="center" cellpadding="0"
		cellspacing="0" class="table" style="margin-top:10px;" id="tableList">
			...
			<tr class="within2bg">
				...
				<td align="center"><a href="supplier_detail.jsp"><%=suppliers.get(i).getName()%></a></td>
				<td align="center"><%=suppliers.get(i).getContactMan()%><br></td>
				<td align="center"><%=suppliers.get(i).getTel()%><br></td>
				<td align="center"><%=suppliers.get(i).getAddress()%></td>
				<td align="center"><%=suppliers.get(i).getEmail()%></td>
				<td align="center"><a
					href="<%=path%>/static/module/book/book002/supplier_modify.jsp?
			id=<%=suppliers.get(i).getId()%>&
			name=<%=suppliers.get(i).getName()%>&
			contactMan=<%=suppliers.get(i).getContactMan()%>&
			tel=<%=suppliers.get(i).getTel()%>&
			address=<%=suppliers.get(i).getAddress()%>&
			email=<%=suppliers.get(i).getEmail()%>&
			remark=<%=suppliers.get(i).getRemark()%>">修改</a>
					&nbsp;&nbsp;&nbsp; <a
					href="<%=path%>/DeleteAllServlet?name=supplier&id=<%=suppliers.get(i).getId()%>">删除</a></td>
			</tr>
			<%
			  }}else{
			%>
	...
	<table width="90%" border="0" align="center" cellpadding="3"
		cellspacing="0">
		<tr>
			<td align="right">共有记录：<%=suppliers.size()%>条&nbsp;每页5条&nbsp;
